...
- config - Configuration files and updates, dependency updates and version bumps
  ...
  "comments": [
    {
      "line": number,
      "end_line": number,
      "issue_type": "failure|bug|security|performance|refactor|idea|bad_practice|other",
      "issue_impact": "critical|high|medium|low",
      "fix_priority": "hotfix|first|second|backlog",
      "model_confidence": "very_high|high|medium|low", 
...
ISSUE TYPE CLASSIFICATION ("issue_type" field):
- failure - System failure or critical issue, MUST be fixed ASAP
- bug - Logical bug or issue in the code, MUST be fixed
- security - Security vulnerability or issue, MUST be fixed
- performance - Performance issue or bottleneck, should be fixed
- refactor - Refactoring opportunity or code improvement, may be fixed
- idea - Idea for improvement or new feature, may be fixed
- bad_practice - Bad practice or anti-pattern, may be fixed
- other - Other issue or suggestion

ISSUE IMPACT CLASSIFICATION ("issue_impact" field):
...
FIX PRIORITY CLASSIFICATION ("fix_priority" field):
- **hotfix** (Very High Priority): MUST be fixed ASAP
- **first** (High Priority): Must be fixed in the next release
- **second** (Medium Priority): Should be fixed if there are resources
- **backlog** (Low Priority): Should be fixed later if there are no resources now

MODEL CONFIDENCE LEVELS ("model_confidence" field):
...
- **low** (40-60%): Potential issue or general suggestion for improvement, need to look at the code more closely

VERIFICATION CHECKLIST:
✅ Each issue has a clear business impact
✅ EACH ENUM VALUE IS CORRECT
